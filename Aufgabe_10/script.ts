...
function updateCounter(): void {
    ...
    for (var a:number = 0; a < list.checked.length; a++)
    if (list.checked[a]==true){number1++}
    else {number2++};
    counterDOMElement.innerHTML = list.text.length + " in total       -"+ number1 + " done         - " + number2 + " not done";
}
...
function addTodo(): void {
    ...
    if (inputDOMElement.value != "") {
         ...
         */
        list.text.push(inputDOMElement.value);
...
function toggleCheckState(index: number): void {
     ...
     */
    list.checked[index] = !list.checked[index];
